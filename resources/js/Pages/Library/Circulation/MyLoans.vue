...
const getStatusClass = (status) => {
    const map = {
        ...
        returned: 'bg-gray-100 text-gray-650 border border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700',
    };
...
const getProgressDetails = (loan) => {
    if (loan.status === 'returned') {
        return { percent: 100, color: 'bg-gray-300 dark:bg-gray-700', text: 'Returned', bg: 'bg-gray-100 text-gray-500' };
    }
    const now = new Date();
        ...
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium">Manage your active checkouts, renewals, and history</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-10">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
                ...
                <section class="space-y-6">
                    ...
                    <div v-if="activeLoans.length === 0" class="text-center py-16 bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-gray-800 flex flex-col items-center">
                        <div class="w-16 h-16 bg-gray-50 dark:bg-gray-800 rounded-3xl flex items-center justify-center text-gray-400 dark:text-gray-650 mb-4">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                        </div>
                        <h4 class="text-base font-bold text-gray-900 dark:text-gray-150">No Active Loans</h4>
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div 
                            ...
                            :key="loan.id"
                            class="group bg-white dark:bg-gray-900 rounded-[2.5rem] p-6 border border-gray-100 dark:border-gray-800 hover:border-indigo-500/30 dark:hover:border-indigo-500/30 shadow-xl shadow-gray-200/50 dark:shadow-none hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
                        >
                            <div class="flex gap-5 items-start">
                                <!-- Book Cover Thumbnail -->
                                <div class="w-20 h-28 shrink-0 rounded-2xl bg-gray-50 dark:bg-gray-850 border border-gray-100 dark:border-gray-800 overflow-hidden relative shadow-sm">
                                ...
                                <div class="flex-1 min-w-0 space-y-1.5">
                                    ...
                                    <h4 class="text-sm font-black text-gray-900 dark:text-white leading-snug truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-450 transition-colors text-left">
                                        {{ loan.copy.book.title }}
                                    </h4>
                                    <p class="text-[10px] font-mono text-gray-400 dark:text-gray-500 text-left">Barcode: {{ loan.copy.barcode || loan.copy.tracking_hash.substring(0, 12) }}</p>
                                    
                                    <div class="flex items-center gap-4 text-[10px] text-gray-500 font-medium">
                                        <div>
                                            <span class="text-gray-400">Borrowed:</span> {{ new Date(loan.borrowed_at).toLocaleDateString() }}
                                        </div>
                                        <div>
                                            <span class="text-gray-400">Due On:</span> {{ new Date(loan.due_on).toLocaleDateString() }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 pt-5 border-t border-gray-50 dark:border-gray-800 space-y-2">
                                <div class="flex justify-between items-center text-[10px] font-bold">
                                    ...
                                    <span class="text-gray-400 uppercase tracking-widest font-black">
                                        {{ Math.round(getProgressDetails(loan).percent) }}% time elapsed
                                    </span>
                                </div>
                                <div class="w-full h-2 bg-gray-50 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner">
                                    <div 
                            ...
                            <div class="mt-4 flex justify-end">
                                <button
                                    ...
                                    class="w-full sm:w-auto px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white disabled:bg-gray-100 disabled:text-gray-400 dark:disabled:bg-gray-800 dark:disabled:text-gray-600 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:scale-100 shadow-md shadow-indigo-600/10 disabled:shadow-none"
                                >
                ...
                <section class="space-y-6">
                    <div class="flex items-center gap-4">
                        <h3 class="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.3em] whitespace-nowrap">Borrowing History</h3>
                        <div class="h-px w-full bg-gradient-to-r from-gray-200 to-transparent dark:from-gray-800/50"></div>
                    </div>

                    <div v-if="previousLoans.length === 0" class="text-center py-10 bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-850 text-gray-400 dark:text-gray-500 text-xs italic">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                <thead>
                                    <tr class="bg-gray-50/50 dark:bg-gray-800/30">
                                        ...
                                        <th class="px-8 py-4 text-left text-[9px] font-black text-gray-400 uppercase tracking-[0.25em]">Barcode</th>
                                        <th class="px-8 py-4 text-left text-[9px] font-black text-gray-400 uppercase tracking-[0.25em]">Borrowed</th>
                                        <th class="px-8 py-4 text-left text-[9px] font-black text-gray-400 uppercase tracking-[0.25em]">Returned</th>
                                        <th class="px-8 py-4 text-left text-[9px] font-black text-gray-400 uppercase tracking-[0.25em]">Fine Settle</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                    <tr v-for="loan in previousLoans" :key="loan.id" class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors duration-200">
                                        <td class="px-8 py-4 text-sm font-bold text-gray-900 dark:text-gray-200 text-left">
                                            {{ loan.copy.book.title }}
                                        </td>
                                        <td class="px-8 py-4 text-xs font-mono text-gray-400 dark:text-gray-500 text-left">
                                            {{ loan.copy.barcode || loan.copy.tracking_hash.substring(0, 10) }}
                                        </td>
                                        <td class="px-8 py-4 text-xs text-gray-650 dark:text-gray-400 text-left">
