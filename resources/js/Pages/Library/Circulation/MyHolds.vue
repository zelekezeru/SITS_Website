...
const getStatusClass = (status) => {
    const map = {
        ...
        cancelled: 'bg-slate-100 text-slate-500 border border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700',
    ...
    <AuthenticatedLayout>
        <template #header>
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium">Track your reservations and collection queue status</p>
            </div>
        </template>
        ...
        <div class="py-10">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
                ...
                <section class="space-y-6">
                    ...
                    <div v-if="activeHolds.length === 0" class="text-center py-16 bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-slate-800 flex flex-col items-center">
                        <div class="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-3xl flex items-center justify-center text-slate-400 dark:text-slate-655 mb-4">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                        </div>
                        <h4 class="text-base font-bold text-slate-900 dark:text-slate-150">No Active Holds</h4>
                        ...
                        >
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <span class="text-[10px] text-slate-400 font-bold">Placed: {{ new Date(hold.placed_at).toLocaleDateString() }}</span>
                                </div>
                                
                                <div class="space-y-1">
                                    <h4 class="text-base font-black text-slate-900 dark:text-white leading-snug group-hover:text-indigo-650 dark:group-hover:text-indigo-400 transition-colors text-left">
                                        {{ hold.book.title }}
                                    </h4>
                            ...
                            <div class="mt-6 pt-5 border-t border-slate-50 dark:border-slate-800 space-y-4">
                                <!-- Stepper Lines -->
                                <div class="relative flex items-center justify-between">
                                    <div class="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-0.5 bg-slate-100 dark:bg-slate-800 z-0"></div>
                                    <div 
                                    ...
                                    <div class="relative z-10 flex flex-col items-center">
                                        <div class="w-6 h-6 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[10px] font-black border-4 border-white dark:border-slate-900">1</div>
                                        <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase">Reserved</span>
                                    </div>
                                    <!-- Step 2 -->
                                    <div class="relative z-10 flex flex-col items-center">
                                        <div 
                                            class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border-4 border-white dark:border-slate-900 transition-colors"
                                            :class="hold.status === 'ready' ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-600 dark:bg-indigo-950 dark:text-indigo-400'"
                                        >2</div>
                                        <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 mt-1 uppercase">In Queue</span>
                                    </div>
                                    <!-- Step 3 -->
                                    <div class="relative z-10 flex flex-col items-center">
                                        <div 
                                            class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black border-4 border-white dark:border-slate-900 transition-all duration-300"
                                            :class="hold.status === 'ready' ? 'bg-emerald-500 text-white animate-pulse shadow-md shadow-emerald-500/20' : 'bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-600'"
                                        >3</div>
                                        <span class="text-[9px] font-bold mt-1 uppercase" :class="hold.status === 'ready' ? 'text-emerald-500 font-black' : 'text-slate-400 dark:text-slate-500'">Ready</span>
                                    </div>
                                </div>
                ...
                <section class="space-y-6">
                    <div class="flex items-center gap-4">
                        <h3 class="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] whitespace-nowrap">Reservation History</h3>
                        <div class="h-px w-full bg-gradient-to-r from-slate-200 to-transparent dark:from-slate-800/50"></div>
                    </div>

                    <div v-if="historicalHolds.length === 0" class="text-center py-10 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-850 text-slate-400 dark:text-slate-500 text-xs italic">
                            ...
                            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                                <thead>
                                    <tr class="bg-slate-50/50 dark:bg-slate-800/30">
                                        ...
                                        <th class="px-8 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.25em]">Pickup Campus</th>
                                        <th class="px-8 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.25em]">Date Placed</th>
                                        <th class="px-8 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.25em]">Status</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                    <tr v-for="hold in historicalHolds" :key="hold.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors duration-200">
                                        <td class="px-8 py-4 text-sm font-bold text-slate-900 dark:text-slate-200 text-left">
                                            {{ hold.book.title }}
                                        </td>
                                        <td class="px-8 py-4 text-xs font-semibold text-slate-650 dark:text-slate-400 text-left">
