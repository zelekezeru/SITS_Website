...
const personalChips = computed(() => {
    ...
    return [
        { label: 'Active loans', value: p.active_loans ?? 0, route: 'library.my.loans', color: 'text-indigo-600 dark:text-indigo-400' },
        { label: 'Due soon', value: p.due_soon ?? 0, route: 'library.my.loans', color: 'text-amber-600 dark:text-amber-400' },
        { label: 'Overdue', value: p.overdue ?? 0, route: 'library.my.loans', color: 'text-red-600 dark:text-red-400' },
        { label: 'Holds', value: p.holds ?? 0, route: 'library.my.holds', color: 'text-green-600 dark:text-green-400' },
        { label: 'Fines due', value: (p.fines_due ? `${sym} ${p.fines_due}` : `${sym} 0`), route: 'library.my.fines', color: 'text-rose-600 dark:text-rose-400' },
    ];
});
...
const quickActions = computed(() => {
    const all = [
        { label: 'Browse Catalog',   route: 'library.catalog.index',       icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253', permission: 'view_books',       color: 'indigo' },
        { label: 'External Library', route: 'library.sso.redirect', params: { destination: 'library' }, target: '_blank', icon: 'M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14', permission: 'view_books', color: 'sky' },
        ...
        { label: 'Process Returns',  route: 'library.circulation.returns',  icon: 'M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6', permission: 'return_book',      color: 'green' },
        { label: 'Add Book',         route: 'library.books.create',         icon: 'M12 4v16m8-8H4', permission: 'create_book',      color: 'violet' },
        { label: 'Scan & Place',     route: 'library.scan.place',           icon: 'M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z', permission: 'create_book',      color: 'amber' },
        { label: 'Transfers',        route: 'library.transfers.index',      icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4', permission: 'request_transfer', color: 'sky' },
        { label: 'Add Resource',     route: 'library.admin.resources.create', icon: 'M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z', permission: 'manage_external_links', color: 'blue' },
        { label: 'Digital Archive',  route: 'library.archive.index',        icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', permission: 'view_secure_pdf',  color: 'rose' },
        { label: 'Manage Users',     route: 'library.users.index',          icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', permission: 'manage_users',     color: 'purple' },
        { label: 'Campuses',         route: 'library.campuses.index',       icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', permission: 'manage_campus',    color: 'teal' },
    ];
...
const selfServiceCards = [
    { label: 'My Loans',  route: 'library.my.loans',  icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2',   desc: 'View your current checkouts & due dates' },
    { label: 'My Holds',  route: 'library.my.holds',  icon: 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9', desc: 'Track your reserved items' },
    { label: 'Resources', route: 'library.resources.index', icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1', desc: 'Access databases & online tools' },
];
