  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-lg sm:text-xl text-gray-800 dark:text-gray-200 leading-tight truncate max-w-[200px] sm:max-w-none">
            {{ secureDocument.title }}
            </h2>
        </div>
        <div class="flex flex-wrap items-center gap-2 sm:gap-4">
            <!-- Zoom Controls -->
            <div class="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-1 border dark:border-gray-700">
                <button @click="updateScale(-0.25)" class="p-1.5 hover:bg-white dark:hover:bg-gray-700 rounded text-gray-700 dark:text-gray-300" title="Zoom Out">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" /></svg>
                </button>
                <button @click="setAutoFit" class="px-2 text-[10px] font-bold uppercase text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400" title="Fit to Width">
                    Fit
                </button>
                <button @click="updateScale(0.25)" class="p-1.5 hover:bg-white dark:hover:bg-gray-700 rounded text-gray-700 dark:text-gray-300" title="Zoom In">
            ...
            <!-- Page Controls -->
            <div class="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-1 border dark:border-gray-700">
                <button @click="changePage(-1)" class="p-1.5 hover:bg-white dark:hover:bg-gray-700 rounded shadow-sm disabled:opacity-50 text-gray-700 dark:text-gray-300 transition" :disabled="page <= 1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                </button>
                <span class="px-3 text-xs font-bold text-gray-900 dark:text-gray-100 min-w-[50px] text-center">{{ page }} / {{ totalPages }}</span>
                <button @click="changePage(1)" class="p-1.5 hover:bg-white dark:hover:bg-gray-700 rounded shadow-sm disabled:opacity-50 text-gray-700 dark:text-gray-300 transition" :disabled="page >= totalPages">
    ...
    <div class="py-6 sm:py-12 select-none" oncopy="return false" oncut="return false" oncontextmenu="return false">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-center overflow-x-auto">
        ...
        <div ref="canvasContainer" class="border dark:border-gray-800 rounded-lg shadow-2xl bg-white dark:bg-gray-900 overflow-hidden transition-all duration-300"></div>
      </div>
    </div>
  </AuthenticatedLayout>
</template>
