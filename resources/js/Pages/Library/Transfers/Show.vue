    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="font-black text-2xl text-slate-900 dark:text-white leading-tight tracking-tight">Transfer #{{ transfer.id }}</h2>
        ...
        <div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8 space-y-8">
            <!-- Main Info Card -->
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-slate-800 p-8 grid grid-cols-1 md:grid-cols-2 gap-10 relative overflow-hidden transition-all">
                <div class="absolute -right-10 -top-10 w-40 h-40 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none"></div>
                
                <div class="relative z-10">
                    <div class="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl mb-6 inline-block">
                    ...
                    <h3 class="text-2xl font-black text-slate-900 dark:text-white leading-tight mb-2">{{ transfer.copy.book.title }}</h3>
                    <p class="text-xs text-slate-400 dark:text-slate-500 font-mono tracking-tighter uppercase">ID: {{ transfer.copy.tracking_hash.substring(0, 16) }}</p>
                    
                    <div class="mt-8 space-y-4">
                        <div class="flex flex-col gap-1">
                            <span class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Reason / Justification</span>
                ...
                <div class="space-y-4 text-sm relative z-10">
                    <div class="bg-slate-50 dark:bg-slate-800/30 rounded-3xl p-6 space-y-3 border border-slate-100 dark:border-slate-800/50">
                        <p class="flex justify-between items-center"><strong class="text-[10px] uppercase tracking-widest text-slate-400">Origin</strong> <span class="font-bold text-slate-900 dark:text-slate-100">{{ transfer.from_campus.name }}</span></p>
                        <p class="flex justify-between items-center"><strong class="text-[10px] uppercase tracking-widest text-slate-400">Destination</strong> <span class="font-bold text-slate-900 dark:text-slate-100">{{ transfer.to_campus.name }}</span></p>
                        <div class="h-px bg-slate-200 dark:bg-slate-700 my-2"></div>
                        <p class="flex justify-between items-center"><strong class="text-[10px] uppercase tracking-widest text-slate-400">Requester</strong> <span class="font-medium text-slate-700 dark:text-slate-300">{{ transfer.requester.name }}</span></p>
                        <p v-if="transfer.approved_by" class="flex justify-between items-center"><strong class="text-[10px] uppercase tracking-widest text-slate-400">Approver</strong> <span class="font-medium text-slate-700 dark:text-slate-300">{{ transfer.approver?.name }}</span></p>
                        <p v-if="transfer.dispatched_by" class="flex justify-between items-center"><strong class="text-[10px] uppercase tracking-widest text-slate-400">Dispatcher</strong> <span class="font-medium text-slate-700 dark:text-slate-300">{{ transfer.dispatcher?.name }}</span></p>
                        <p v-if="transfer.received_by" class="flex justify-between items-center"><strong class="text-[10px] uppercase tracking-widest text-slate-400">Receiver</strong> <span class="font-medium text-slate-700 dark:text-slate-300">{{ transfer.receiver?.name }}</span></p>
            ...
            <!-- Actions Card -->
            <div v-if="transfer.status === 'requested' || transfer.status === 'approved' || transfer.status === 'in_transit'" class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-slate-800 p-8 transition-all">
                <h3 class="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-8 flex items-center gap-4">
                    Management Actions
                ...
                <div v-if="transfer.status === 'requested' && permissions.includes('approve_transfer')" class="space-y-6">
                    ...
                    <div class="flex gap-2 max-w-lg">
                        <input v-model="actionNote" placeholder="Reason for rejection..." class="flex-1 rounded-2xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 text-sm focus:ring-2 focus:ring-red-500 transition-all" />
                ...
                <div v-if="transfer.status === 'approved' && permissions.includes('approve_transfer')" class="flex gap-2 max-w-xl">
                    <input v-model="courierRef" placeholder="Courier Tracking / Reference Number..." class="flex-1 rounded-2xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 transition-all" />
                ...
                <div v-if="transfer.status === 'in_transit'" class="space-y-8">
                    ...
                    <div v-if="permissions.includes('approve_transfer')" class="pt-8 border-t dark:border-slate-800 space-y-6">
                        <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-[0.3em]">Exception Handling</p>
                        <div class="flex flex-wrap gap-3">
                            <input v-model="actionNote" placeholder="Incident report / Note..." class="flex-1 min-w-[300px] rounded-2xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 text-sm focus:ring-2 focus:ring-indigo-500 transition-all" />
