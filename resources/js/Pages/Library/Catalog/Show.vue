    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div class="flex items-center gap-4 min-w-0">
                    <Link :href="route('library.catalog.index')" class="p-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 text-gray-500 hover:text-indigo-600 transition shadow-sm active:scale-90">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                    </Link>
                    <div class="truncate">
                        <h2 class="font-black text-2xl text-gray-900 dark:text-white leading-tight tracking-tight truncate">
                            {{ book.title }}
                        </h2>
                    </div>
                </div>
                <div v-if="$page.props.auth.permissions.includes('edit_book')" class="flex shrink-0">
                    <Link :href="route('library.books.edit', book.id)" class="px-6 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest hover:bg-gray-50 dark:hover:bg-gray-700 transition shadow-sm active:scale-95">
                        {{ __('Edit Title') }}
                    </Link>
                </div>
            </div>
        ...
        <div class="py-10">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
                ...
                <!-- Title Details -->
                <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-2xl shadow-gray-200/50 dark:shadow-none rounded-[2.5rem] border border-transparent dark:border-gray-800 flex flex-col lg:flex-row p-1 gap-1">
                    <!-- Cover Section -->
                    <div class="lg:w-1/3 p-6 flex items-center justify-center">
                        <div class="w-full max-w-[280px] aspect-[3/4] bg-gradient-to-tr from-gray-100 to-gray-50 dark:from-gray-850 dark:to-gray-800 rounded-[2.5rem] flex items-center justify-center text-gray-300 dark:text-gray-650 border border-gray-200/50 dark:border-gray-800 relative overflow-hidden shadow-xl hover:shadow-2xl hover:scale-102 hover:rotate-1 transition-all duration-500 group">
                            ...
                            <!-- Badge -->
                            <div class="absolute top-4 right-4 px-3 py-1 bg-white/85 dark:bg-black/55 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-widest text-indigo-750 dark:text-indigo-300 border border-white/25 dark:border-gray-800 shadow-sm">
                                {{ book.language }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:w-2/3 p-8 lg:pl-4">
                        ...
                        <h1 class="text-4xl font-black text-gray-900 dark:text-white mb-2 leading-tight">{{ book.title }}</h1>
                        <div v-if="book.average_rating" class="flex items-center gap-2 mb-6">
                            <div class="flex items-center text-amber-500">
                                <svg v-for="i in 5" :key="i" class="w-4 h-4" :class="i <= Math.round(book.average_rating) ? 'fill-current' : 'text-gray-300 dark:text-gray-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span class="text-sm font-black text-gray-900 dark:text-gray-100">{{ book.average_rating }} / 5</span>
                            <span class="text-xs text-gray-400 font-bold">({{ book.reviews?.filter(r => r.is_approved).length ?? 0 }} {{ __('reviews') }})</span>
                        </div>
                        <p v-if="book.subtitle" class="text-xl text-gray-500 dark:text-gray-400 mb-8 font-medium leading-relaxed">{{ book.subtitle }}</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
                            <div class="space-y-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-[10px] uppercase tracking-widest font-bold text-gray-400 mb-0.5">{{ __('Author(s)') }}</div>
                                        <div class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ book.authors.map(a => a.name).join(', ') }}</div>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg bg-gray-50 dark:bg-gray-800 flex items-center justify-center text-gray-400">
                                    ...
                                    <div>
                                        <div class="text-[10px] uppercase tracking-widest font-bold text-gray-400 mb-0.5">{{ __('ISBN') }}</div>
                                        <div class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ book.isbn || __('NOT ASSIGNED') }}</div>
                                    </div>
                                </div>
                            </div>
                            <div class="space-y-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-[10px] uppercase tracking-widest font-bold text-gray-400 mb-0.5">{{ __('Publication') }}</div>
                                        <div class="text-sm font-bold text-gray-900 dark:text-gray-100">
                                            {{ book.publisher || 'N/A' }} 
                                            <span v-if="book.published_year" class="text-gray-400 font-medium">({{ book.published_year }})</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-[10px] uppercase tracking-widest font-bold text-gray-400 mb-0.5">{{ __('Edition') }}</div>
                                        <div class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ book.edition || __('1st Edition') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        
                        <div class="p-6 rounded-[2rem] bg-gray-50 dark:bg-gray-800/30 text-sm text-gray-600 dark:text-gray-400 leading-relaxed italic mb-8 border border-gray-100 dark:border-gray-800">
                        ...
                        <div class="mb-8 p-6 rounded-[2.5rem] bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100/50 dark:border-indigo-900/35 space-y-4">
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    <h4 class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-wider">{{ __('Reserve this Book') }}</h4>
                                    <p class="text-[11px] text-gray-500 dark:text-gray-450 font-medium">{{ __('Place a hold request and collect at your preferred campus') }}</p>
                                </div>
                            </div>
                            
                            <form @submit.prevent="submitHold" class="flex flex-col sm:flex-row gap-3 items-end">
                                <div class="w-full flex-1">
                                    <label class="block text-[10px] font-bold text-gray-450 dark:text-gray-500 uppercase tracking-widest mb-1.5">{{ __('Pickup Campus') }}</label>
                                    <select 
                                        v-model="holdForm.campus_id" 
                                        required
                                        class="w-full py-3 px-4 bg-white dark:bg-gray-950 border border-gray-205 dark:border-gray-800 rounded-2xl text-xs font-bold text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                                    >
                        ...
                        <div v-if="book.secure_documents?.length && $page.props.auth.permissions.includes('view_secure_pdf')" class="flex flex-wrap gap-4">
                            <div v-for="doc in book.secure_documents" :key="doc.id" class="flex items-center gap-3">
                                ...
                                <button 
                                    ...
                                    class="p-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl text-gray-500 hover:text-indigo-600 transition shadow-sm active:scale-90"
                ...
                <!-- Copies -->
                <div class="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-xl border border-transparent dark:border-gray-800 overflow-hidden">
                    <div class="p-8 border-b dark:border-gray-800 flex items-center justify-between">
                        <div>
                            <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                            ...
                            <p class="text-xs text-gray-500 mt-1 font-medium">{{ __('Tracking and location status for each copy') }}</p>
                        </div>
                    ...
                    <div v-if="book.copies.length === 0" class="p-16 text-center text-gray-400 dark:text-gray-600 italic">
                    ...
                    <div v-else class="overflow-x-auto">
                        <table class="min-w-full divide-y dark:divide-gray-800">
                            <thead class="bg-gray-50/50 dark:bg-gray-800/30">
                                <tr>
                                    <th class="px-8 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{{ __('Barcode / Hash') }}</th>
                                    <th class="px-8 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{{ __('Location') }}</th>
                                    <th class="px-8 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{{ __('Status') }}</th>
                                    <th class="px-8 py-4 text-right text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{{ __('Actions') }}</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y dark:divide-gray-800">
                                <tr v-for="copy in book.copies" :key="copy.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                                    <td class="px-8 py-5 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-gray-200 font-mono tracking-tighter">
                                        {{ copy.barcode || copy.tracking_hash.substring(0,12) + '...' }}
                                    </td>
                                    <td class="px-8 py-5 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">
                                        <div v-if="copy.shelf_box" class="flex items-center gap-2">
                                            <span class="w-2 h-2 rounded-full bg-indigo-500"></span>
                                            <span class="font-bold text-gray-900 dark:text-gray-200">{{ copy.shelf_box.row.floor.campus.code }}</span>
                                            <span class="text-gray-300 dark:text-gray-700">›</span>
                                            <span class="font-medium uppercase">{{ copy.shelf_box.label }}</span>
                                        </div>
                                        <span v-else class="text-gray-400 dark:text-gray-600 italic">{{ __('Unassigned') }}</span>
                                    </td>
                                    <td class="px-8 py-5 whitespace-nowrap">
                                        <span class="px-3 py-1 inline-flex text-[10px] font-black rounded-lg uppercase tracking-widest border" :class="{
                                            ...
                                            'bg-gray-100 text-gray-600 border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700': ['withdrawn', 'lost'].includes(copy.status)
                                        }">
                                            {{ __(copy.status) }}
                                        </span>
                                    </td>
                                    <td class="px-8 py-5 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        <button 
                                            ...
                                            class="p-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl text-gray-400 hover:text-indigo-600 transition shadow-sm active:scale-90"
                                            :title="__('QR Label')"
                                        >
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>
                                        </button>
                ...
                <!-- Reviews Section -->
                <div class="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-xl border border-transparent dark:border-gray-800 overflow-hidden p-8 space-y-8">
                    <div>
                        <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                            {{ __('Reviews & Ratings') }}
                        ...
                        <p class="text-xs text-gray-500 mt-1 font-medium">{{ __('Hear what other patrons and faculty think about this title') }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-6">
                            <!-- Summary -->
                            <div class="p-6 rounded-[2rem] bg-gray-50 dark:bg-gray-800/30 border border-gray-100 dark:border-gray-800 space-y-4">
                                <h4 class="text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">{{ __('Ratings Overview') }}</h4>
                                <div class="flex items-baseline gap-2">
                                    <span class="text-5xl font-black text-gray-900 dark:text-white">{{ book.average_rating || '0.0' }}</span>
                                    <span class="text-sm text-gray-400 font-bold">/ 5.0</span>
                                </div>
                                <div class="flex items-center text-amber-500">
                                    <svg v-for="i in 5" :key="i" class="w-5 h-5" :class="i <= Math.round(book.average_rating || 0) ? 'fill-current' : 'text-gray-300 dark:text-gray-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.907c.961 0 1.36 1.246.588 1.81l-3.972 2.87a1 1 0 00-.364 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.971-2.87a1 1 0 00-1.175 0l-3.97 2.87c-.783.57-1.838-.197-1.539-1.118l1.518-4.674a1 1 0 00-.364-1.118L2.98 9.42c-.773-.564-.373-1.81.588-1.81h4.906a1 1 0 00.951-.69l1.519-4.674z" />
                                    </svg>
                                </div>
                                <p class="text-xs text-gray-500 font-medium">{{ __('Based on') }} {{ book.reviews?.filter(r => r.is_approved).length ?? 0 }} {{ __('approved reviews') }}</p>
                            </div>

                            <!-- Write/Edit Review Form -->
                            <div class="p-6 rounded-[2rem] bg-gray-50 dark:bg-gray-800/30 border border-gray-100 dark:border-gray-800 space-y-4">
                                <h4 class="text-sm font-black uppercase tracking-wider text-gray-700 dark:text-gray-300">
                                    {{ myReview ? __('Edit Your Review') : __('Write a Review') }}
                                </h4>
                                <form @submit.prevent="submitReview" class="space-y-4">
                                    <!-- Interactive Star Rating selection -->
                                    <div class="space-y-1">
                                        <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest">{{ __('Your Rating') }}</label>
                                        <div class="flex items-center gap-1">
                                            ...
                                            >
                                                <svg 
                                                    class="w-8 h-8" 
                                                    :class="star <= (hoverRating || reviewForm.rating) ? 'fill-current' : 'text-gray-300 dark:text-gray-600'" 
                                                    fill="none" 
                                                    stroke="currentColor" 
                                                    viewBox="0 0 24 24"
                                                >
                                    ...
                                    <!-- Review comment -->
                                    <div class="space-y-1">
                                        <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest">{{ __('Your Review') }}</label>
                                        <textarea 
                                            v-model="reviewForm.review"
                                            rows="4" 
                                            :placeholder="__('What did you think of the writing style, content, or relevance to your studies?...')" 
                                            class="w-full text-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition placeholder-gray-400 text-gray-700 dark:text-gray-300"
                                        ></textarea>
                                    </div>
                        ...
                        <div class="lg:col-span-2 space-y-4">
                            ...
                            <div v-else class="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                                <div 
                                    ...
                                    :key="review.id"
                                    class="p-6 rounded-[2rem] bg-gray-50 dark:bg-gray-800/30 border border-gray-100 dark:border-gray-800 space-y-3 relative group"
                                >
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="text-sm font-bold text-gray-900 dark:text-gray-100">{{ review.user?.name || __('Unknown Patron') }}</div>
                                                <div class="text-[10px] text-gray-400 font-medium">{{ new Date(review.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) }}</div>
                                            </div>
                                        </div>

                                        <div class="flex items-center gap-2">
                                            <div class="flex text-amber-500">
                                                <svg v-for="i in 5" :key="i" class="w-4 h-4" :class="i <= review.rating ? 'fill-current' : 'text-gray-300 dark:text-gray-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.907c.961 0 1.36 1.246.588 1.81l-3.972 2.87a1 1 0 00-.364 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.971-2.87a1 1 0 00-1.175 0l-3.97 2.87c-.783.57-1.838-.197-1.539-1.118l1.518-4.674a1 1 0 00-.364-1.118L2.98 9.42c-.773-.564-.373-1.81.588-1.81h4.906a1 1 0 00.951-.69l1.519-4.674z" />
                                                </svg>
                                            </div>
                                    ...
                                    <div class="flex items-center justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-800 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            v-if="canModerateReviews"
                                            @click="toggleApproval(review.id)"
                                            class="px-3 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-indigo-500 hover:text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all"
                                        >
