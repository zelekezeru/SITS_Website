...
const statusClass = (s) => ({
    ...
    waived: 'bg-gray-100 text-gray-500 border border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700',
    ...
    <AuthenticatedLayout>
        <template #header>
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium">Review pending payments, penalties, and payment history</p>
            </div>
        </template>
        ...
        <div class="mx-auto max-w-4xl p-6 space-y-8">
            ...
            <div
                ...
                :class="outstanding > 0
                    ? 'bg-gradient-to-br from-rose-900 via-rose-950 to-slate-900 border-rose-950/80 dark:border-gray-800'
                    : 'bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 border-indigo-950/80 dark:border-gray-800'"
            >
                <!-- Decos -->
                ...
                <div class="relative z-10 shrink-0">
                    ...
                    <div v-else-if="outstanding === 0" class="px-5 py-3 rounded-2xl bg-emerald-500/20 border border-emerald-500/30 backdrop-blur-md text-xs font-black text-emerald-300 uppercase tracking-widest flex items-center gap-2">
                        <span>All Clear — No Fines Due</span> 🎉
                    </div>
                </div>
            </div>

            <!-- Fines list card -->
            <div class="bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-850 overflow-hidden shadow-xl shadow-gray-200/30 dark:shadow-none">
                <div class="p-6 border-b dark:border-gray-800 flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-wider">Fine Register</h3>
                        <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5 font-medium">History of all fee statements and payments</p>
                    </div>
                </div>

                <div v-if="!fines.data.length" class="px-4 py-20 text-center flex flex-col items-center">
                    <div class="w-16 h-16 bg-gray-50 dark:bg-gray-850 rounded-3xl flex items-center justify-center text-gray-400 dark:text-gray-655 mb-4">
                    ...
                    <p class="text-sm font-bold text-gray-905 dark:text-gray-400">Excellent Record! No Fines Found</p>
                </div>
                ...
                <div v-else class="divide-y divide-gray-100 dark:divide-gray-850">
                    <div
                        ...
                        :key="fine.id"
                        class="p-6 hover:bg-gray-50/50 dark:hover:bg-gray-850/20 transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-6"
                    >
                        <div class="flex items-start gap-4">
                            ...
                            <div class="space-y-1 text-left">
                                ...
                                <p v-if="fine.note" class="text-[11px] text-gray-400 dark:text-gray-500 font-medium">
                                    Note: {{ fine.note }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-5 justify-between sm:justify-end shrink-0">
                            <div class="text-right space-y-0.5">
                                <p class="text-sm font-black text-gray-900 dark:text-white">{{ currency }} {{ money(fine.amount) }}</p>
