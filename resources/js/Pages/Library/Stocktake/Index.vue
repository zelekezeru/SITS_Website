    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h1 class="text-base font-semibold text-slate-900 dark:text-white">Inventory Stocktake</h1>
                <button
        ...
        <div class="p-6 max-w-7xl mx-auto space-y-6">
            ...
            >
                <div v-if="showNewForm" class="rounded-xl border border-indigo-200 dark:border-indigo-800 bg-white dark:bg-slate-900 p-5">
                    <h3 class="text-sm font-semibold text-slate-900 dark:text-white mb-4">Start New Inventory Audit</h3>
                    <form @submit.prevent="startStocktake" class="flex flex-col sm:flex-row gap-4">
                        <div class="flex-1">
                            <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Campus</label>
                            <select v-model="form.campus_id" class="w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <div class="flex-1">
                            ...
                            <input v-model="form.notes" type="text" placeholder="e.g., End-of-semester audit" class="w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 text-sm focus:ring-indigo-500 focus:border-indigo-500" />
                        </div>
                ...
                <table v-if="stocktakes.data?.length" class="w-full text-sm">
                    <thead class="bg-slate-50 dark:bg-slate-800">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Started By</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Date</th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Scans</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Status</th>
                            <th class="px-4 py-3"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <tr v-for="st in stocktakes.data" :key="st.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                            <td class="px-4 py-3 font-medium text-slate-900 dark:text-white">{{ st.campus?.name }}</td>
                            <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ st.starter?.name }}</td>
                            <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ new Date(st.started_at).toLocaleDateString() }}</td>
                            <td class="px-4 py-3 text-right text-slate-700 dark:text-slate-300 font-medium">{{ st.scans_count }}</td>
                            <td class="px-4 py-3">
