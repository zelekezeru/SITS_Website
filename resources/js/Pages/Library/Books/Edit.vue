...
const form = useForm({
    ...
    classification_type: props.book.classification_type || 'dewey',
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                <Link :href="route('library.catalog.show', book.id)" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 transition whitespace-nowrap">&larr; <span class="hidden sm:inline">Back to </span>{{ book.title.substring(0, 20) }}{{ book.title.length > 20 ? '...' : '' }}</Link>
                <h2 class="font-semibold text-xl text-slate-800 dark:text-slate-200 leading-tight border-l pl-3 border-slate-300 dark:border-slate-700">
                    Edit Title
                </h2>
            </div>
        </template>
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input v-model="form.edition" type="text" class="mt-1 block w-full bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" />
                        ...
                        <div class="flex items-center justify-end space-x-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                            <Link :href="route('library.catalog.show', book.id)" class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 font-medium transition">Cancel</Link>
