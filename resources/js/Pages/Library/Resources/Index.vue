...
const props = defineProps({
    resources: Object // grouped by category
})

// QR modal state
const qrModal = ref({ show: false, url: '', title: '' })
const openQr = (res) => {
    qrModal.value = { show: true, url: route('library.resources.qr', res.id), title: res.name }
}
</script>

<template>
    <Head title="External Resources" />
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                ...
                <div v-if="$page.props.auth.permissions.includes('manage_external_links')" class="flex items-center gap-4">
                    <Link 
                        :href="route('library.admin.resources.create')" 
                        class="px-6 py-3 bg-indigo-600 border border-transparent rounded-2xl font-bold text-sm text-white uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/25 active:scale-95"
                    >
                        + Add Resource
                    </Link>
                    <Link 
        ...
        <div class="py-10">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
                ...
                <div v-for="(group, category) in resources" :key="category" class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <div class="flex items-center gap-4">
                        <h3 class="text-xs font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.3em] whitespace-nowrap">{{ category || 'General' }}</h3>
                        ...
                        >
                            <div class="p-8 flex-1">
                                <div class="flex items-start justify-between mb-6">
                                    ...
                                    <span :class="{
                                        'px-3 py-1 text-[10px] uppercase font-black tracking-widest rounded-lg border shadow-sm': true,
                                        'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800': res.access_tier === 'free',
                                        'bg-indigo-50 text-indigo-600 border-indigo-100 dark:bg-indigo-900/20 dark:text-indigo-400 dark:border-indigo-800': res.access_tier === 'premium',
                                        'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800': res.access_tier === 'restricted'
                                    }">
                                        {{ res.access_tier }}
                                    </span>
                                </div>
