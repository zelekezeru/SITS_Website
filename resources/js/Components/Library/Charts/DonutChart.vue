    ...
    <div class="flex items-center gap-5">
        <div class="relative shrink-0">
            <svg viewBox="0 0 100 100" class="h-28 w-28 -rotate-90">
                <circle cx="50" cy="50" :r="R" fill="none" stroke="currentColor" class="text-gray-100 dark:text-gray-800" stroke-width="12" />
                <circle
                    v-for="(a, i) in arcs"
                    :key="i"
                    cx="50" cy="50" :r="R"
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center">
                <span class="text-lg font-bold text-gray-900 dark:text-white">{{ grandTotal }}</span>
        ...
        <ul class="flex-1 space-y-1.5">
            <li v-for="(s, i) in segments" :key="i" class="flex items-center gap-2 text-xs">
                ...
                <span class="flex-1 capitalize text-gray-600 dark:text-gray-400">{{ s.label }}</span>
                <span class="font-medium text-gray-800 dark:text-gray-200">{{ s.value }}</span>
            </li>
        </ul>
    </div>
</template>
